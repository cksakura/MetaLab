...
public class FragpipePeptide extends MetaPeptide {

	private String[] proteins;

	public FragpipePeptide(String sequence, String[] proteins, int[] spCount, double[] intensity) {
		super(sequence);
	...
	public FragpipePeptide(String sequence, String modSeq, int length, int[] charges, int missCleavage, double mass,
			double score, double PEP, String[] proteins, double[] intensity, int[] spCount, int[] idenType) {
		super(sequence, length, charges, missCleavage, mass, score, intensity);
		// TODO Auto-generated constructor stub
		this.PEP = PEP;
		this.modSeq = modSeq;
		this.proteins = proteins;
	...
	public static class XmlFPPepElementParser extends XmlPepElementParser {
		...
		public MetaPeptide parse(Element ePep) {
			...
			String[] spCountSs = ePep.attributeValue("spCounts").split("_");
			int[] spCount = new int[spCountSs.length];
			for (int i = 0; i < spCountSs.length; i++) {
				spCount[i] = Integer.parseInt(spCountSs[i]);
			}
			FragpipePeptide peptide = new FragpipePeptide(sequence, proteins, spCount, intensity);
			return peptide;
		}
	}
