...
public class MetaLabMainFrameDiaRT extends MetaLabMainFrameDia {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MetaLabMainFrameDiaRT frame = new MetaLabMainFrameDiaRT();
	 ...
	 * @throws IOException
	 */
	public MetaLabMainFrameDiaRT() throws URISyntaxException, IOException {
		super();
	}
	...
	protected void initial() throws IOException, URISyntaxException {
		...
		File parameter = new File(ClassLoader.getSystemClassLoader().getResource(".").toURI().getPath(),
				"parameters" + MetaParIODiaRT.versionFile);
		...
		MetaParameterDiaRT par = MetaParIODiaRT.parse(parameter);
		par.setMagDbItems(magDbItems);

		getContentPane().setLayout(new MigLayout("", "[1200:1600:1920,grow]", "[800:900:1080,grow]"));

		this.contentPane = new MetaLabMainPanelDiaRT(par, msv);

		getContentPane().add(contentPane, "cell 0 0,grow");

		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(MetaLabMainFrameDiaRT.class.getResource("/bmi/med/uOttawa/metalab/icon/dia_128.png")));

		setTitle("MetaLab " + MetaParIODiaRT.version);
		...
		mntmLoadParameter.addActionListener(l -> {
			...
			int returnValue = fileChooser.showOpenDialog(MetaLabMainFrameDiaRT.this);
			if (returnValue == JFileChooser.APPROVE_OPTION) {

				File file = fileChooser.getSelectedFile();
				MetaParameterDiaRT loadPar = MetaParIODiaRT.parse(file);
				if (loadPar.getWorkflowType() == MetaLabWorkflowType.DiaNNMAGRT) {
					MetaLabMainFrameDiaRT.this.getContentPane().remove(this.contentPane);
					contentPane = new MetaLabMainPanelDiaRT(loadPar, msv);
					MetaLabMainFrameDiaRT.this.getContentPane().add(contentPane, "cell 0 0,grow");
					MetaLabMainFrameDiaRT.this.validate();
		...
		mntmSaveParameter.addActionListener(l -> {
			...
			int value = fileChooser.showSaveDialog(MetaLabMainFrameDiaRT.this);
			...
			if (value == JFileChooser.APPROVE_OPTION) {
				...
				((MetaLabMainPanelDiaRT) contentPane).updateParameter();
				MetaParameter savePar = ((MetaLabMainPanelDiaRT) contentPane).getParameter();

				MetaParIODiaRT.export((MetaParameterDiaRT) savePar, fileToSave);
			}
		});
		...
		mntmHelp.setIcon(
				new ImageIcon(MetaLabMainFrameDiaRT.class.getResource("/toolbarButtonGraphics/general/Help16.gif")));
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				((MetaLabMainPanelDiaRT) contentPane).updateParameter();
				MetaParameter par = ((MetaLabMainPanelDiaRT) contentPane).getParameter();

				MetaParIODiaRT.export((MetaParameterDiaRT) par, parameter);

				MagDbItem[][] magDbItems = ((MetaParameterDiaRT) par).getMagDbItems();
