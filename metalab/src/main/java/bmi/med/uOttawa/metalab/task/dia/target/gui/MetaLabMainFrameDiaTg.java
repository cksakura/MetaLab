...
import bmi.med.uOttawa.metalab.task.dia.gui.MetaLabSourceDialogDia;
...
public class MetaLabMainFrameDiaTg extends MetaLabMainFrameDia {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MetaLabMainFrameDiaTg frame = new MetaLabMainFrameDiaTg();
	 ...
	 * @throws IOException
	 */
	public MetaLabMainFrameDiaTg() throws URISyntaxException, IOException {
		super();
	}
	...
	protected void initial() throws IOException, URISyntaxException {
		...
		this.contentPane = new MetaLabMainPanelDiaTg(par, msv);

		getContentPane().add(contentPane, "cell 0 0,grow");

		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(MetaLabMainFrameDiaTg.class.getResource("/bmi/med/uOttawa/metalab/icon/dia_128.png")));
		...
		mntmLoadParameter.setIcon(
				new ImageIcon(MetaLabMainFrameDiaTg.class.getResource("/toolbarButtonGraphics/general/Open16.gif")));
		...
		mntmLoadParameter.addActionListener(l -> {
			...
			int returnValue = fileChooser.showOpenDialog(MetaLabMainFrameDiaTg.this);
			if (returnValue == JFileChooser.APPROVE_OPTION) {

				File file = fileChooser.getSelectedFile();
				MetaParameterDia loadPar = MetaParaIoDia.parse(file);
				if (loadPar.getWorkflowType() == MetaLabWorkflowType.DiaNNMAGRT) {
					MetaLabMainFrameDiaTg.this.getContentPane().remove(this.contentPane);
					contentPane = new MetaLabMainPanelDiaTg(loadPar, msv);
					MetaLabMainFrameDiaTg.this.getContentPane().add(contentPane, "cell 0 0,grow");
					MetaLabMainFrameDiaTg.this.validate();

				} else {
					JOptionPane.showMessageDialog(this,
							"Only parameters with workflow type " + MetaLabWorkflowType.DiaNNMAGRT.getDescription()
		...
		mntmSaveParameter.setIcon(
				new ImageIcon(MetaLabMainFrameDiaTg.class.getResource("/toolbarButtonGraphics/general/Save16.gif")));
		mnFile.add(mntmSaveParameter);
		mntmSaveParameter.addActionListener(l -> {
			JFileChooser fileChooser = new JFileChooser();
			fileChooser.setDialogTitle("Save parameter file");

			int value = fileChooser.showSaveDialog(MetaLabMainFrameDiaTg.this);
			...
			if (value == JFileChooser.APPROVE_OPTION) {
				...
				((MetaLabMainPanelDiaTg) contentPane).updateParameter();
				MetaParameter savePar = ((MetaLabMainPanelDiaTg) contentPane).getParameter();
		...
		mntmExit.setIcon(
				new ImageIcon(MetaLabMainFrameDiaTg.class.getResource("/toolbarButtonGraphics/general/Stop16.gif")));
		...
		mntmVerify.setIcon(
				new ImageIcon(MetaLabMainFrameDiaTg.class.getResource("/toolbarButtonGraphics/general/Add16.gif")));
		...
		mntmResource.setIcon(new ImageIcon(
				MetaLabMainFrameDiaTg.class.getResource("/toolbarButtonGraphics/general/Bookmarks16.gif")));
		...
		mntmHelp.setIcon(
				new ImageIcon(MetaLabMainFrameDiaTg.class.getResource("/toolbarButtonGraphics/general/Help16.gif")));
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				((MetaLabMainPanelDiaTg) contentPane).updateParameter();
				MetaParameter par = ((MetaLabMainPanelDiaTg) contentPane).getParameter();

				MetaParaIoDia.export((MetaParameterDia) par, parameter);

				MagDbItem[][] magDbItems = ((MetaParameterDia) par).getMagDbItems();
