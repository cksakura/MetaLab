...
public class MetaLabMainFrameHGM extends MetaLabPFindMainFrame {
	...
	private MetaLabMainPanelHGM contentPane;
	 ...
	 * @throws IOException
	 */
	public MetaLabMainFrameHGM() throws URISyntaxException, IOException {
		super();
	}
	...
	protected void initial() throws IOException, URISyntaxException {
		...
		File parameter = new File(ClassLoader.getSystemClassLoader().getResource(".").toURI().getPath(),
				"parameters_" + MetaParaIOHGM.versionFile);

		File resources = new File(ClassLoader.getSystemClassLoader().getResource(".").toURI().getPath(),
				"resources_" + MetaParaIOHGM.versionFile);

		MetaSourcesHGM msv2 = MetaSourcesIoHGM.parse(resources);
		MetaParameterHGM parV3 = MetaParaIOHGM.parse(parameter);

		getContentPane().setLayout(new MigLayout("", "[1200:1600:1920,grow 600]", "[800:900:1080,grow]"));
		
		this.contentPane = new MetaLabMainPanelHGM(parV3, msv2);
		
		getContentPane().add(contentPane, "cell 0 0,grow");
		
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(MetaLabMainFrameHGM.class.getResource("/bmi/med/uOttawa/metalab/icon/HGM.png")));

		setTitle("MetaLab " + MetaParaIOHGM.version);
		...
		mntmLoadParameter.setIcon(
				new ImageIcon(MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Open16.gif")));
		...
		mntmLoadParameter.addActionListener(l -> {
			...
			int returnValue = fileChooser.showOpenDialog(MetaLabMainFrameHGM.this);
			if (returnValue == JFileChooser.APPROVE_OPTION) {

				File file = fileChooser.getSelectedFile();
				MetaParameterHGM loadPar = MetaParaIOHGM.parse(file);
				if (loadPar.getWorkflowType() == MetaLabWorkflowType.pFindHGM) {
					
					MetaLabMainFrameHGM.this.getContentPane().remove(this.contentPane);
					contentPane = new MetaLabMainPanelHGM(loadPar, msv2);
					MetaLabMainFrameHGM.this.getContentPane().add(contentPane, "cell 0 0,grow");
					MetaLabMainFrameHGM.this.validate();

				} else {
					JOptionPane.showMessageDialog(this,
							"Only parameters with workflow type " + MetaLabWorkflowType.pFindHGM.getDescription()
		...
		mntmSaveParameter.setIcon(
				new ImageIcon(MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Save16.gif")));
		mnFile.add(mntmSaveParameter);
		mntmSaveParameter.addActionListener(l -> {
			JFileChooser fileChooser = new JFileChooser();
			fileChooser.setDialogTitle("Save parameter file");

			int value = fileChooser.showSaveDialog(MetaLabMainFrameHGM.this);
			...
			if (value == JFileChooser.APPROVE_OPTION) {
				...
				((MetaLabMainPanelHGM) contentPane).updateParameter();
				MetaParameter par = ((MetaLabMainPanelHGM) contentPane).getParameter();

				MetaParaIOHGM.export((MetaParameterHGM) par, fileToSave);
			}
		});

		JMenuItem mntmExit = new JMenuItem("Exit");
		mntmExit.setIcon(
				new ImageIcon(MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Stop16.gif")));
		...
		mntmVerify.setIcon(
				new ImageIcon(MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Add16.gif")));
		...
		mntmDownloadDb.setIcon(
				new ImageIcon(MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Import16.gif")));
		mntmDownloadDb.addActionListener(l -> {
			HGMDownloadFrame frame = new HGMDownloadFrame();
			frame.setAlwaysOnTop(true);
			frame.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
			frame.setVisible(true);
		});
		...
		mntmResource.setIcon(new ImageIcon(
				MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Bookmarks16.gif")));
		mntmResource.addActionListener(l -> {
			MetaLabSourcePathDialog dialog = new MetaLabSourceDialogHGM(msv2, resources, this);
		...
		mntmHelp.setIcon(
				new ImageIcon(MetaLabMainFrameHGM.class.getResource("/toolbarButtonGraphics/general/Help16.gif")));
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				MetaParaIOHGM.export((MetaParameterHGM) par, parameter);
