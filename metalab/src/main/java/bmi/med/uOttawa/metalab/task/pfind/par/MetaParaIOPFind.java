 ...
 *
 */
public class MetaParaIOPFind {

	private static Logger LOGGER = LogManager.getLogger(MetaParaIOPFind.class);

	public static MetaParameterPFind parse(String json) {
		return parse(new File(json));
	}

	public static MetaParameterPFind parse(File json) {
	...
	public static MetaParameterPFind parse(JSONObject obj) {
		...
		boolean openSearch = obj.has("isOpenSearch") ? obj.getBoolean("isOpenSearch") : true;
		...
		MetaOpenPar mosp = new MetaOpenPar(openSearch, openPsmFDR, openProFDR, matchBetweenRuns);
		...
		MetaParameterPFind parV3 = new MetaParameterPFind(metaPar, fixMods, variMods, enzymeString, missCleavages,
				digestMode, quanMode, isobaric, isobaricTag, combineLabel, isoCorFactor, openSearch, mosp, mptp);
	...
	public static void exportBlank(File out) {
		...
		jw.key("version").value(MetaLabTask.version);
		...
		MetaOpenPar osp = MetaOpenPar.getDefault();
		jw.key("isOpenSearch").value(osp.isOpenSearch());
		jw.key("openPsmFDR").value(osp.getOpenPsmFDR());
		jw.key("openProFDR").value(osp.getOpenProFDR());
		jw.key("mbr").value(osp.isMatchBetweenRuns());
	...
	public static void export(MetaParameterPFind par, String out) {
		export(par, new File(out));
	}

	public static void export(MetaParameterPFind par, File out) {
		...
		MetaSsdbCreatePar mscp = par.getMscp();
		jw.key("ssdb").value(mscp.isSsdb());
		jw.key("cluster").value(mscp.isCluster());
		jw.key("ssOpen").value(mscp.isOpen());
		jw.key("xtandemFDR").value(mscp.getXtandemFDR());
		...
		MetaOpenPar osp = par.getMop();
		jw.key("isOpenSearch").value(osp.isOpenSearch());
		jw.key("openPsmFDR").value(osp.getOpenPsmFDR());
		jw.key("openProFDR").value(osp.getOpenProFDR());
		jw.key("mbr").value(osp.isMatchBetweenRuns());

		MetaPep2TaxaPar mptp = par.getPtp();
		jw.key("buildIn").value(mptp.isBuildIn());
		jw.key("unipept").value(mptp.isUnipept());
